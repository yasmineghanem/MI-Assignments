...
def calculate_gpa(student: Student, courses: List[Course]) -> float:
    ...
    hours_sum = sum([course.hours for course in courses if student.id in course.grades])
    hours_grades_sum = sum([course.hours * course.convert_grade_to_points(course.grades[student.id]) for course in courses if student.id in course.grades])
    
    return 0.0 if hours_sum == 0  else hours_grades_sum/hours_sum